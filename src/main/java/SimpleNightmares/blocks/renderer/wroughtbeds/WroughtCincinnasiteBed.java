...
public class WroughtCincinnasiteBed extends BlockBed {
    ...
    public WroughtCincinnasiteBed() {
        super();
        setTranslationKey(SimpleNightmares.MODID + ".wroughtcincinnasitebed");     // Used for localization and naming (en_us.lang)
        setRegistryName("wroughtcincinnasitebed");        // The unique name (within the mod) that identifies this block
    ...
    public void onBlockPlacedBy(World worldIn, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack) {
        ...
        state = state.withProperty(FACING, enumfacing).withProperty(PART, EnumPartType.FOOT);
        if (worldIn.isAirBlock(pos.offset(enumfacing))) {
            worldIn.setBlockState(pos, state, 10);
            worldIn.setBlockState(pos.offset(enumfacing), state.withProperty(PART, EnumPartType.HEAD), 10);
        } else {
    ...
    public TileEntity createNewTileEntity(final World worldIn, final int meta) {
        return new TileEntityWroughtCincinnasiteBed();
    }
    @Override
    ...
    public void dropBlockAsItemWithChance(World worldIn, BlockPos pos, IBlockState state, float chance, int fortune) {
        if (!worldIn.isRemote && !state.getValue(PART).equals(EnumPartType.FOOT)) {

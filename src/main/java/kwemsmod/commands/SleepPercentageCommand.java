...
import static kwemsmod.proxy.CommonProxy.config;
...
public class SleepPercentageCommand extends CommandBase {
    ...
    public void onPlayerSleep(MinecraftServer server, String playername) {
        ...
        if (onlinePlayers >= 2 && enableChatFeedback) {
            ...
            if (sleepPlayers != -1) {
                ITextComponent message = new TextComponentString(TextFormatting.GOLD + playername + " is now sleeping (" + sleepingPlayers + "/" + onlinePlayers + ")" + " (" + togo + ")" + TextFormatting.RESET);
                server.getPlayerList().getPlayers().forEach(player -> player.sendMessage(message));
            }
            else {
                ITextComponent message = new TextComponentString(TextFormatting.GOLD + playername  + " is now sleeping (" + sleepingPlayers + "/" + onlinePlayers + ")" + " (" + percentage + "%)" + TextFormatting.RESET);
                server.getPlayerList().getPlayers().forEach(player -> player.sendMessage(message));
            }
            if (enableChatRemarks) {
                if (sleepPlayers != -1) {
                    if (sleepingPlayers >= sleepPlayers) {
                        ...
                        ITextComponent dayMessage = new TextComponentString(getRandomMessage());
